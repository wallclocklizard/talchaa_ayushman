...
const WindowAdapterRcOpaque *slint_live_preview_window(const LiveReloadingComponentInner *component);
...
Box<Value> slint_interpreter_value_new_data_transfer(const DataTransfer *data);

const DataTransfer *slint_interpreter_value_to_data_transfer(const Value *val);

/// Construct a new Value containing a Keys
Box<Value> slint_interpreter_value_new_keys(const Keys *keys);

const Keys *slint_interpreter_value_to_keys(const Value *val);

/// Construct a new Value containing a StyledText
Box<Value> slint_interpreter_value_new_styled_text(const StyledText *text);

const StyledText *slint_interpreter_value_to_styled_text(const Value *val);
